"""scaffold.yaml parsing and validation."""
...
def load_config(path: Path = Path("scaffold.yaml")) -> ScaffoldConfig:
    """Load and validate scaffold.yaml."""
    if not path.exists():
        raise ConfigError(
            f"Config file not found: {path}\n\n"
            "Fix: Run 'scaffold init' to create a scaffold.yaml, "
...
def find_eval(config: ScaffoldConfig, eval_name: str) -> EvalConfig:

"""Convert Promptfoo configs to scaffold.yaml."""
...
def import_promptfoo_config(
    source: Path, output: Path = Path("scaffold.yaml")
) -> None:
    ...
    if raw.get("prompts"):
        ...
        if isinstance(prompts, list) and len(prompts) == 1:
            ...
            warnings.append(
                ...
                "Scaffold uses a single prompt file."
            )
            prompt_file = prompts[0]
...
def _convert_providers(raw: dict, warnings: list[str]) -> dict:
    """Convert promptfoo providers to scaffold target config."""
...
def _convert_tests(raw: dict, warnings: list[str]) -> tuple[list[dict], str | None]:
    """Convert promptfoo tests to scaffold evals."""
    tests = raw.get("tests", [])
    if not tests:
        return [
            {
    ...
    if raw.get("redTeam"):
        warnings.append("Red teaming plugins are not supported by Scaffold.")

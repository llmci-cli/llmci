"""Interactive llmci.yaml generator."""
...
def run_init() -> None:
    """Run the interactive llmci init flow."""
    if Path("llmci.yaml").exists():
        if not click.confirm("llmci.yaml already exists. Overwrite?", default=False):
            click.echo("Aborted.")
            return
    ...
    Path("llmci.yaml").write_text(yaml_text)
    click.echo("\nCreated llmci.yaml")
